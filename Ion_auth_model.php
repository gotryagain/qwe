...
class Ion_auth_model extends CI_Model
{
	 ...
	 **/
	 ...
	 public function checkCompanyCode($CCode){
		 $query = $this->db->query("SELECT * FROM company WHERE company_code = '$CCode'");
		 if($query->num_rows() > 0){
			 return TRUE;
		 } else {
			 return FALSE;
	 ...
	 public function getCompanyID($CCode){
		$query = $this->db->query("SELECT company_id FROM company WHERE company_code = '$CCode'");
		$row = $query->row(); 
	 ...
	 public function getUserRole($id){
		$query = $this->db->query("SELECT GR.name AS GRPNAME FROM users_groups AS UG
...
WHERE UG.user_id = $id");
	...
	**/
	...
	public function getIndustry(){
		$this->db->select('industry_id, active, industry, industry_desc');
		$this->db->from('industries');
		$this->db->where('industry_id !=', 99999);
	...
	public function getMyIndDrop($UID){
		$this->db->select('I.industry_id, I.industry, UI.Is_Primary');
		...
		$this->db->where('UI.industry_id !=', 99999);
		$this->db->order_by('industry_id', 'asc');
		$query = $this->db->get();
		return $query->result_array();
	}
	public function resetPrimService($where, $data){
		$this->db->update($this->IndDrop, $data, $where);
		return $this->db->affected_rows();
	}
	public function updatePrimService($where, $data){
		$this->db->update($this->IndDrop, $data, $where);
		return $this->db->affected_rows();
	}
	/* Show my primary sector */
	public function PrimSector($id){
		$this->db->select('I.industry_id, I.industry');
		$this->db->from('user_industries AS UI');
		$this->db->join('industries AS I', 'I.industry_id = UI.industry_id', 'inner');
	...
	public function IndustrySetCheck($id){

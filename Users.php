...
class Users extends CI_Controller {
	...
	public function industry_selection(){
		
		//Clear existing Primary
		$data = array(
			'Is_Primary' => 0,
		);
		$this->Users_model->resetPrimService(array('user_id' => $this->input->post('user_id')), $data);
		...
		$insert = $this->Users_model->save_selection($selections);
		echo json_encode(array("status" => TRUE));
	}
	public function industry_selection_remove(){
		
		$type = $this->input->post();
		$user_id = $this->input->post('user_id');
		$industry_id = $this->input->post('industry_id');

		$insert = $this->Users_model->remove_selection($user_id, $industry_id);
		echo json_encode(array("status" => TRUE));
	}
	
	public function update_bio(){
		
		$config['upload_path']          = './manager/uploads/biopics/';
		...
		if (empty($_FILES['userfile']['name'])) {
			...
			$update = $this->Users_model->updateUser(array('id' => $this->input->post('user_id')), $data);
			redirect('/user-info/'.$this->input->post('user_id').'', 'refresh');
			...
			if ( !$this->upload->do_upload('userfile')){
				...
				$bioPicture = $_FILES["userfile"]["name"];
				$data = array(
					'bio_image' => $bioPicture,
					'bio_text' => $this->input->post('bio_text'),
				);
				$update = $this->Users_model->updateUser(array('id' => $this->input->post('user_id')), $data);
				redirect('/user-info/'.$this->input->post('user_id').'', 'refresh');
